...
namespace _043___REF___Ключевое_слово
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
             ...
             * INFO:
             *  1. Переменная ОБЯЗАТЕЛЬНО должна быть ИНИЦИАЛИЗИРОВАННА при передаче через REF.
             *  2. НЕ обязательно присваивать новое значение переменной в методе.
             *  3. REF нельзя присвоить базовое значение внутри параметров принимаемых методом
             *      void TEST(ref int x = 0) - ОШИБКА
             *  4. Методы принимающие значения с модификатором ref, in, out не могут быть асинхронными.
             *      async void TEST(ref int x) - НЕ скомпилируется
             * 
             * Чтоже это значит:
             ...
             * По сути ключевое слово ref, позволяет изменить поведение переменных
             * значимого типа, а точнее сделать его похожим на поведение ссылочных типов.
             */
            int a = 2;
            ...
            // будут возвращать сразу несколько значений.
            int c = 0;
            int d = 0;
            int e = 0;
             ...
             * хранить ссылку.
             *
             * Для того чтобы уничтожить или изменить ссылку необходимо передать её с
             * помощью ref.
             * 
             * Аналогичное поведение будет происходить в том случае если мы будем
             * переопределять старый массив - новым массивом при передаче старого
             ...
             * возвращаемого значения и также укажем ref при вызове ключевого слова
